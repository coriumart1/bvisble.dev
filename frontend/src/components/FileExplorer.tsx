...
export default function FileExplorer() {
  ...
  const load = async () => {
    try {
      ...
      setAllFolders(af)
      setFolders(af.filter(f => f.parent_id === folderId))
      const ad = await api.documents.getAll()
      setDocs(ad.filter(d => d.folder_id === folderId))
    } catch {}
  }
  ...
  const onCtx = (e: React.MouseEvent, type: ContextMenu['type'], item?: Folder | Document) => {
    e.preventDefault()
    e.stopPropagation()
    // Adjust position so menu doesn't go off-screen
    const menuW = 200, menuH = type === 'empty' ? 80 : 110
    const x = e.clientX + menuW > window.innerWidth ? e.clientX - menuW : e.clientX
    const y = e.clientY + menuH > window.innerHeight ? e.clientY - menuH : e.clientY
    setCtx({ x, y, type, item })
  }

  const createFolder = async () => {
    setCtx(null)
    try {
      const f = await api.folders.create({ name: 'Neuer Ordner', parent_id: folderId })
      await load()
  ...
  const createDoc = async () => {
    setCtx(null)
    try {
      const d = await api.documents.create({ title: 'Neues Dokument', folder_id: folderId })
      await load()
    ...
    <div className="flex flex-col h-full bg-gray-950 text-gray-100 select-none overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-700 bg-gray-900 shrink-0">
        <button
          ...
          className="p-1.5 rounded hover:bg-gray-700 disabled:opacity-30 disabled:cursor-not-allowed text-gray-400 transition-colors"
        >
        ...
        <div className="flex-1 flex items-center gap-0.5 bg-gray-800 rounded px-3 py-1.5 text-sm overflow-x-auto">
          {crumbs.map((c, i) => (
            <span key={i} className="flex items-center gap-0.5 shrink-0">
              ...
              <button
                onClick={() => navigateCrumb(c, i)}
                className={`hover:text-blue-400 transition-colors whitespace-nowrap ${
                  i === crumbs.length - 1 ? 'text-gray-100 font-medium' : 'text-gray-400'
                }`}
              >
                {i === 0 && (
        ...
        {isEmpty ? (
          ...
          <div className="grid gap-1" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(90px, 1fr))' }}>
            ...
            {folders.map(f => (
              <div
                key={`folder-${f.id}`}
                className="flex flex-col items-center gap-1.5 p-2 rounded-lg cursor-pointer hover:bg-gray-800 active:bg-gray-700 transition-colors group"
                onDoubleClick={() => navigate(f)}
                onContextMenu={(e) => onCtx(e, 'folder', f)}
              >
                <div className="text-5xl leading-none drop-shadow-sm">📁</div>
                {renamingFolder?.id === f.id ? (
                  ...
                  <span className="text-xs text-center text-gray-300 leading-tight line-clamp-2 w-full break-words">
                    {f.name}
                  </span>
                )}
              </div>
            ...
            {docs.map(d => (
              <div
                key={`doc-${d.id}`}
                className="flex flex-col items-center gap-1.5 p-2 rounded-lg cursor-pointer hover:bg-gray-800 active:bg-gray-700 transition-colors"
                onDoubleClick={() => openDocument(d)}
                onContextMenu={(e) => onCtx(e, 'document', d)}
              >
                <div className="text-5xl leading-none drop-shadow-sm">📄</div>
                {renamingDoc?.id === d.id ? (
                  <input
      ...
      {ctx && (
        ...
        >
          {ctx.type === 'empty' && (
            <>
              <button className="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-700 text-left rounded-t-lg" onClick={createFolder}>
                <span>📁</span> <span>Neuer Ordner</span>
              </button>
              <button className="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-700 text-left rounded-b-lg" onClick={createDoc}>
                <span>📄</span> <span>Neues Dokument</span>
              </button>
            </>
          )}

          {ctx.type === 'folder' && ctx.item && (() => {
            const f = ctx.item as Folder
            return (
              <>
                <button className="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-700 text-left" onClick={() => { setCtx(null); navigate(f) }}>
                  <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                  <span>Öffnen</span>
                </button>
                <div className="border-t border-gray-600 my-1" />
                <button className="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-700 text-left" onClick={() => { setCtx(null); setRenamingFolder({ id: f.id, name: f.name }) }}>
                ...
                <button className="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-700 text-left text-red-400" onClick={() => deleteFolder(f)}>
          ...
          {ctx.type === 'document' && ctx.item && (() => {
            const d = ctx.item as Document
            return (
              <>
                <button className="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-700 text-left" onClick={() => openDocument(d)}>
                  <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                  <span>Öffnen</span>
                </button>
                <div className="border-t border-gray-600 my-1" />
                <button className="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-700 text-left" onClick={() => { setCtx(null); setRenamingDoc({ id: d.id, title: d.title }) }}>
                  <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                  <span>Umbenennen</span>
                </button>
                <button className="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-700 text-left text-red-400" onClick={() => deleteDoc(d)}>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                  <span>Löschen</span>
                </button>
              </>
      ...
      {openDoc && (
        <div className="fixed inset-0 z-40 flex" onClick={() => { load(); setOpenDoc(null) }}>
          ...
          <div
            className="relative flex flex-col bg-gray-900 w-full max-w-4xl mx-auto my-8 rounded-xl shadow-2xl overflow-hidden"
            onClick={e => e.stopPropagation()}
          >
            ...
            <div className="flex-1 overflow-auto">
              <DocumentEditor
                ...
                document={openDoc}
                folders={allFolders}
                onUpdate={(updated) => setOpenDoc(updated)}
              />
            </div>
          </div>
        </div>
